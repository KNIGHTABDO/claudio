...
3. **Conversational Turns vs. Music Changes**: If the user is just chatting with you (e.g., asking "who are you?", "whats this about?", "tell me a joke", or clarifying tastes) and does NOT ask for a new song or vibe change, do NOT return any tracks in the \`play\` list (leave it as an empty array: \`"play": []\`). Crucially, if the user asks about the *currently playing song* (which is active in the environment context), do NOT put it in the \`play\` list, as they are already listening to it! Only populate the \`play\` list when they explicitly request new music, or when you decide it is the perfect time to transition the vibe to a different track.
